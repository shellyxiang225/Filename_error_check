...
class FileSearchEngine(ttk.Frame):
    ...
    def __init__(self, master):
        ...
        self.term_var = ttk.StringVar(value='如xls')
    ...
    def create_term_row(self):
        ...
        term_lbl = ttk.Label(term_row, text="筛选", width=8)
    ...
    def create_check_button(self):
        """Add check button to labelframe"""
        ...
        search_btn.pack(side=BOTTOM, padx=5)
    ...
    def on_check(dir):
        '''check the filename correct or not'''

        FolderPath = filedialog.askdirectory()
        dir = FolderPath
        '''root indicates the root directory, dir indicates the subdirectory, and file name'''
        list = []

        for root, dirs, file in os.walk(dir):
            '''Iterate through the file list to get the file name'''
            for d in file:
                'filename+K3 CODE'
        ...
        ws.cell(row=1, column=4).value = '超链接'
        ws['A1'].font = font
        ws['A1'].fill = fill
        ...
        '''Every 3 data in the list is stored in excel'''
        for i in range(0, len(list), 3):
            row = [list[i], list[i + 1],list[i+2]]
            sheet.append(row)
